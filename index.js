...
function customAssign(...objects) {
  ...
  if(arguments.length<2 ){console.log("Помилка: Має бути передано принаймні два об'єкти.")}// Перевірка, чи передано достатньо об'єктів якщо ні повертаємо "Помилка: Має бути передано принаймні два об'єкти."
  else{const sum=Object.assign(...objects);// Використання методу `assign` для об'єднання об'єктів
  return sum;// Перетворення числових значень на рядки, ітеруємо об'єкт за допомогою for in
...
function customEntries(obj) {
  if(!(typeof(obj)==='object')){console.log("Помилка: Аргумент не є об'єктом.")}// Перевірка, чи переданий аргумент є об'єктом, якщо ні повертаємо "Помилка: Аргумент не є об'єктом."
  else{
...
function customObjectFromEntries(entries) {
  if(!Array.isArray(entries)){console.log("Помилка: Вхідний аргумент має бути масивом.")} // Перевірка, чи вхідний аргумент є масивом,якщо ні повертаєм "Помилка: Вхідний аргумент має бути масивом."
  else{const arr1=entries.map(([key, value])=>typeof(key)==='number'?[key=toString(key),value]:[key,value]);// Використання методу `map` для обробки значень властивостей
  console.log(arr1);
...
function checkProperty(obj, prop) {
  if(!(typeof(obj)==='object')){return false}// Перевіряємо, чи вхідний параметр obj є об'єктом, якщо ні, повертаємо false
  else if(!(typeof(prop)==='string')){return false}// Перевіряємо, чи вхідний параметр prop є рядком, якщо ні, повертаємо false
  else{return obj.hasOwnProperty(prop)}// Використовуємо метод hasOwnProperty() для перевірки наявності властивості в об'єкті
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  else{obj1=Object.assign(obj1,obj2);// Використовуємо метод Object.assign() для додавання властивостей obj2 до obj1
...
function keysAndValues(obj) {
  if(!(typeof(obj)==='object')){return null}// Перевіряємо, чи вхідний параметр є об'єктом, якщо ні, повертаємо null
  else{ const keys=Object.keys(obj);// Використовуємо метод Object.keys() для отримання масиву ключів об'єкта
  const values=Object.values(obj);// Використовуємо метод Object.values() для отримання масиву значень об'єкта
  const entries=Object.entries(keys,values);// Повертаємо масив з масивами ключів та значень
return [keys, values];
}}
...
function filterObjectsByKey(arr, key) {
  ...
  else{ let newArr=Array();
    for(let i=0;i<arr.length;i++){
   if(arr[i].hasOwnProperty(key)){ newArr.push(arr[i])}}
  return newArr;// Використовуємо метод filter() для вибірки об'єктів, що містять вказаний ключ
...
function checkAndPreventExtensions(obj) {
  if(typeof(obj)==='object'){return false}// Перевіряємо, чи вхідний параметр є об'єктом, якщо ні, повертаємо false
  else{if(Object.isExtensible(obj))if(Object.isExtensible(obj)){Object.preventExtensions(obj);};// Перевіряємо, чи можна додати нові властивості до об'єкта
  if(!Object.isExtensible(obj)){return Object.isExtensible(obj)}// Якщо можна, запобігаємо подальшому додаванню властивостей
...
function sealAndCheck(obj) {
  if(!(typeof(obj)==='object')){return false}// Перевіряємо, чи вхідний параметр є об'єктом, якщо ні, повертаємо false
  else{obj=Object.seal(obj);// Запечатовуємо об'єкт
...
function checkOwnership(obj1, obj2, key) {
  if(obj1.hasOwnProperty(key)&&obj2.hasOwnProperty(key)){// Використовуємо метод hasOwnProperty() для перевірки наявності ключа в обох об'єктах.
  if(obj1.key===obj2.key){return true}// Якщо обидва об'єкти мають вказаний ключ повертаємо результат порівняння їх значень.
  else{return false}}// Якщо значення однакові, повернемо true.
...
function getObjectValuesSum(obj) {
  if(!(typeof(obj))){return 0}// Перевірка, чи є аргумент об'єктом,якщо ні повертаємо 0
  else{let arr=Object.values(obj);// Отримуємо всі значення об'єкта
  let red=arr.reduce((ak,el)=>ak+el);// Обчислюємо суму значень
  return red;}// Повертаємо суму
}
...
function convertArrayToObj(arr) {
  ...
  else{let obj={};// Створюємо пустий об'єкт який записуємо в змінну
  for(let i=0;i<arr.length;i++){
      let key, value;
      key=arr[i][0]; value=arr[i][1];
      if(obj.hasOwnProperty(key)){console.log(`У масиві є дубльований ключ: ${key}`)}
      else{obj=Object.fromEntries(arr)}
  ...
  return obj// Додаємо ключ та значення до об'єкта
